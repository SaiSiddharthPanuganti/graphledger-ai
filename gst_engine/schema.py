"""
Deliverable 1 — GST Knowledge Graph Schema
============================================
...
GST PRIMER:
  ...
  EWB    = E-Way Bill (required for goods movement > ₹50,000)
...
Why graph?
  Multi-hop ITC validation requires traversal:
    Recipient GSTIN → Invoice → Supplier GSTIN → Return (GSTR-1)
    → GSTR-2B → GSTR-3B → TaxPayment
  SQL needs O(n²) JOINs per hop; graph traversal is O(k).
...
class GSTINNode(TypedDict):
    """
    GST Identification Number node — one taxpayer can have multiple GSTINs
...
class InvoiceNode(TypedDict):
    """
    Tax Invoice — the core document in GST ITC chain.
    B2B invoices above ₹5L require IRN (e-invoice mandate).
...
class IRNNode(TypedDict):
    """
    Invoice Reference Number — cryptographic proof of e-invoice.
    Generated by IRP (Invoice Registration Portal) as SHA-256 hash of:
      SupplierGSTIN + DocType + DocNo + DocDate + RecipientGSTIN
    Mandatory for B2B invoices where supplier turnover > ₹5 Crore.
    """
...
class ReturnNode(TypedDict):
    """
    GST Return filing — monthly/quarterly compliance document.
...
class EWayBillNode(TypedDict):
    """
    E-Way Bill — mandatory electronic permit for goods movement.
    Required when consignment value > ₹50,000.
...
class TaxPaymentNode(TypedDict):
    """
    Tax Payment Challan — proof of GST deposit to government.
...
MISMATCH_TAXONOMY: Dict[str, Dict[str, Any]] = {
    ...
    "EXTRA_IN_2B": {
        ...
        "legal_reference":  "Rule 36(1) CGST Rules — ITC documentary requirements",
    ...
    "GSTIN_MISMATCH": {
        ...
        "legal_reference":  "Section 16(1) CGST Act — registered taxable person requirement",
    ...
    "DATE_MISMATCH": {
        ...
        "legal_reference":  "Section 16(4) CGST Act — time limit for ITC claim",
    ...
    "IRN_MISMATCH": {
        ...
        "risk_level":       "CRITICAL",
        "itc_impact":       True,
        "itc_risk_multiplier": 1.5,   # Highest multiplier — potential fraud
        "description":      (
            ...
            "Invalid IRN = invoice may be fabricated. CRITICAL risk — potential fraud."
        ),
        "legal_reference":  "Rule 48(4) CGST Rules + Section 122(1)(ii) — fraudulent ITC",
    ...
    "EWAYBILL_MISSING": {
        ...
        "description":      (
            "Goods movement above ₹50,000 without valid E-Way Bill. "
...
# ═══════════════════════════════════════════════════════════════
# SCHEMA REGISTRY — for graph builders to reference
# ═══════════════════════════════════════════════════════════════
...
SCHEMA_DESCRIPTION = "GST Knowledge Graph Schema — India's GST ITC Reconciliation Engine"
...
# ═══════════════════════════════════════════════════════════════
# DEMO — run this file to see schema summary
# ═══════════════════════════════════════════════════════════════

if __name__ == "__main__":
    print("=" * 60)
    print("  GraphLedger AI — GST Knowledge Graph Schema v1.0")
    print("=" * 60)
    ...
    print(f"\n🔗 EDGE TYPES ({len(EDGE_SCHEMA)}):")
    for name, meta in EDGE_SCHEMA.items():
        props = meta["properties"] or ["none"]
        print(f"  {name:<25} {meta['source']:<15} → {meta['target']:<15}  props: {props}")

    print(f"\n⚠️  MISMATCH TAXONOMY ({len(MISMATCH_TAXONOMY)} types):")
    ...
    print(f"\n✅ Schema loaded successfully into empty NetworkX DiGraph")
